...
class ReferenceProperty(ndb.KeyProperty):
    """
    from http://dylanv.org/2012/08/22/a-hitchhikers-guide-to-upgrading-app-engine-models-to-ndb/
    KeyProperty does not automatically fetch the referred-to entity from the datastore. 
    This is a custom ndb.Property subclass to emulate the old ReferenceProperty:
    """
...
class Group(ndb.Model):
    ...
    can_view = ndb.BooleanProperty(default=False)
    can_edit = ndb.BooleanProperty(default=False)
    can_upload= ndb.BooleanProperty(default=False)
    can_administer = ndb.BooleanProperty(default=False)
    ...
    def get_default_group(cls):
        #the default user is named 'viewer'
        defaultGroup =  cls.query(cls.name == 'viewer').get()
        if not defaultGroup:
            #add the default Group if necessary
            defaultGroup = cls(name="viewer", can_view=True)
            defaultGroup.put()
        return defaultGroup
...
class User(User):
    ...
    group = ReferenceProperty(kind=Group, default=Group.get_default_group())
